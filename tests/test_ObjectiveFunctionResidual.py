...
from OptClim2 import Parameter, ObjectiveFunctionResidual
...
def rundir(tmpdir_factory):
    res = tmpdir_factory.mktemp("ofResidual")
    return res
...
def objectiveA(rundir, paramsA):
    return ObjectiveFunctionResidual(rundir, paramsA)
...
def test_set_result(objectiveA, valuesA, resultA):
    ...
    assert numpy.all(objectiveA.get_result(valuesA) == resultA)
    assert objectiveA._num_residuals == resultA.size
...
def test_call(objectiveA, valuesA, resultA):
    assert numpy.all(
        objectiveA(list(valuesA.values()), numpy.array([])) == resultA)
